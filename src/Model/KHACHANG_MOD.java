...
public class KHACHANG_MOD {
	public static String taoMaKhachHang() {
		...
		ResultSet rs;
		try {
			rs = kn.getConnection().prepareCall("{call sp_TruyVan_KhachHang}")
					.executeQuery();
	...
	public static boolean themKhachHang(KHACHHANG_OBJ obj) {
		...
		KETNOI_MOD kn = new KETNOI_MOD();
		try {
			CallableStatement cs = kn.getConnection().prepareCall(
	...
	public static boolean xoaKhachHang(String maKH) {
		...
		try {
			cs = kn.getConnection().prepareCall("{call sp_Xoa_KhachHang(?)}");
			cs.setString(1, maKH);
			cs.executeUpdate();
			return true;
		} catch (SQLException e) {
	...
	public static ResultSet timKhachHang(String TuKhoa) {
		...
		CallableStatement cs;
		try {
			cs = kn.getConnection().prepareCall("{call sp_Tim_KhachHang(?)}");
	...
	public static int kiemTraTrungTenDangNhap(String TenDNKH) {
		...
		CallableStatement cs;
		try {
			cs = kn.getConnection().prepareCall("{call sp_KiemTraTrungTenDangNhap_KhachHang(?)}",ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
	...
	public static int kiemTraTrungEmail(String EmailKH) {
		...
		CallableStatement cs;
		try {
			cs = kn.getConnection().prepareCall("{call sp_KiemTraTrungEmail_KhachHang(?)}",ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
	...
	public static ResultSet layToanBoDanhSachKhachHang() {
		...
		CallableStatement cs;
		try {
			cs = kn.getConnection().prepareCall("{call sp_TruyVan_KhachHang}");
			rs = cs.executeQuery();
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public static ResultSet layKhachHang_TheoMa(String MaKH) {
		...
		CallableStatement cs;
		try {
			cs = kn.getConnection().prepareCall(
	...
	public static ResultSet layDanhSachTheoTrang(int Tranghientai,int Sobai_trang)//Danh sach san pham theo trang
	{
		...
		CallableStatement cs;
		try {
			cs = kn.getConnection().prepareCall("{call sp_PhanTrang_KhachHang(?,?)}");
